...
const DELETE_CARD = gql`
  mutation DeleteCard($cardId: ID!, $sectionId: ID!) {
    deleteCard(request: { cardId: $cardId, sectionId: $sectionId }) {
      id
    }
  }
`;

const UPDATE_CARD_TEXT = gql`
  mutation UpdateCardText($cardId: ID!, $newText: String!) {
    updateCardText(request: { cardId: $cardId, newText: $newText }) {
      id
      title
      label
      pos
...
const CardContainer = ({ item, boards }) => {
  ...
  const [deleteCardPos] = useMutation(DELETE_CARD);
  const [updateCardText] = useMutation(UPDATE_CARD_TEXT);
  ...
  useEffect(() => {
    if (cardAdded) {
      if (item.id === cardAdded.sectionId) {
        setCards((prevCards) => [...prevCards, cardAdded]);
  ...
  const onUpdateCardText = (cardId, newText) => {
    updateCardText({
      variables: {
        cardId,
        newText,
    ...
    setCards((prevCards) =>
        prevCards.map((card) =>
            card.id === cardId ? { ...card, title: newText, label: newText } : card
        )
    );
  };
      ...
      <Wrapper className={"card-container"}>
        <WrappedSection>
          ...
          <CardsContainer>
            ...
            >
              {cards.map((card) => (
                <Card key={card.id} card={card}  onUpdateCardText={onUpdateCardText}/>
              ))}
            </Container>
            {isTempCardActive ? (
              <CardComposerDiv>
